...
public class Reporter : MonoBehaviour {
	...
	public void Show(bool showLog,bool showWarning,bool showError)
	{
		if (!this.show)
		{
	...
	public void SaveLog()
	{
		string path = Application.persistentDataPath + "/" + DateTime.Now.ToString("yyyy-MM-dd-HH-mm-ss") + ".log";
		using (StreamWriter sw = new StreamWriter(new FileStream(path, FileMode.Create)))
		{
			foreach (var item in logs)
			{
				sw.WriteLine(item.logType + " \r\ncondition:" + item.condition + "\r\nstacktrace:" + item.stacktrace);
			}
		}
	}
	...
	void doShow(){
		show = true; 
		currentView = ReportView.Logs ;
		gameObject.AddComponent(typeof(ReporterGUI));
	...
	void Update()
	{
		...
		float elapsed = realtimeSinceStartup - lastUpdate ;
		fps = 1f / elapsed ;
		...
		float elapsed2 = realtimeSinceStartup - lastUpdate2 ;
		if( elapsed2 > 1)
		{
			lastUpdate2 = realtimeSinceStartup;
	...
	/*IEnumerator readInfo(  )
	{
		...
		{
			Debug.LogError( www.error + " ->" + prefFile);
		}
		else 
		{
			buildDate = www.text ;
